...
CREATE UNIQUE INDEX [idx_role_name] ON [identity_role] ([normalized_name]) WHERE [normalized_name] IS NOT NULL;
GO

CREATE INDEX [idx_identity_identity_role_claim_role_id] ON [identity_role_claim] ([role_id]);
GO

CREATE INDEX [idx_email] ON [identity_user] ([normalized_email]);
GO

CREATE UNIQUE INDEX [idx_user_name] ON [identity_user] ([normalized_user_name]) WHERE [normalized_user_name] IS NOT NULL;
GO

CREATE INDEX [idx_identity_identity_user_claim_user_id] ON [identity_user_claim] ([user_id]);
GO

CREATE INDEX [idx_identity_identity_user_login_user_id] ON [identity_user_login] ([user_id]);
GO

CREATE INDEX [idx_identity_identity_user_role_role_id] ON [identity_user_role] ([role_id]);
GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20220521210341_AddIdentity', N'5.0.5');
GO
...
ALTER SCHEMA [Security] TRANSFER [identity_user_token];
GO

ALTER SCHEMA [Security] TRANSFER [identity_user_role];
GO

ALTER SCHEMA [Security] TRANSFER [identity_user_login];
GO

ALTER SCHEMA [Security] TRANSFER [identity_user_claim];
GO

ALTER SCHEMA [Security] TRANSFER [identity_user];
GO

ALTER SCHEMA [Security] TRANSFER [identity_role_claim];
GO

ALTER SCHEMA [Security] TRANSFER [identity_role];
GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20220522003935_AddSchemaIdentity', N'5.0.5');
GO
